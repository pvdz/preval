...
function FunctionExpression(node) {
  ...
  return (node.async ? 'async ' : '') + 'function' + (node.generator ? '*' : '') + (node.id ? ' ' + $(node.id) : '') + '(' + node.params.map((node) => {
    let r = $(node);

    // Add typing annotation to function params when not normalized (and available)
    if (printVarTyping && node.type === 'Identifier' && node.name) {
      const meta = printVarTypeFdata.globallyUniqueNamingRegistry.get(node.name);
      if (meta?.typing) {
    ...
    if (meta?.typing) {
      ...
      if (init.type === 'FunctionExpression') {
        ...
        return (
          ...
          (
            meta.callerArgs ? '/*:(' + (init.params.map((_,i) => meta.callerArgs?.[i] || 'unknown').join(', ')) + `)=>${meta.typing.returns?Array.from(meta.typing.returns).join(','):''}*/ ` : ''
          ) + '= ' +
          $(init) +
        ...
        return (
          ...
          (
            // A "primitive" just tells us it can't spy and that it's an immutable value
            (meta.typing.mustBeType || meta.typing.mustBePrimitive) ? '/*:' + (meta.typing.mustBeType || 'primitive') + '*/ ' : ''
          ) + '= ' +
          $(init) +
          (fromFor ? '' : ';') // normalized code cant have `for` but whatever
        );
...
function $(node, _, __, fromFor) {
  ...
  if (!type) {
    console.log();
    console.log('IS THIS A STRING? This node might be incorrect?');
    console.log();
