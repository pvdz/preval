...
function BlockStatement(node) {
  ...
  if (printPids && node.$p?.npid && !/^\$\d*$/.test(node.name)) {
    return `/*${node.$p.npid}~${node.$p.lastPid}*/{\n${node.body.map((n,i) => {
      return '/* stmt(' + n.$p.npid + '): */ ' + $(n,i);
    }).join('\n')}\n}`;
  }
  return '{\n' + node.body.map($).join('\n') + '\n}';
}
...
function FunctionExpression(node) {
  ...
  return (node.async ? 'async ' : '') +
    'function' + (node.generator ? '*' : '') + (node.id ? ' ' + $(node.id) : '') +
    ...
    (printVarTyping&&node.$p.readsArgumentsAny?'/*uses arguments*/ ':'') +
    (printVarTyping&&node.$p.thisAccess?'/*uses this*/ ':'') +
...
function Identifier(node) {
  ...
  if (printPids && node.$p?.npid && !/^\$\d*$/.test(node.name)) {
    return `/*___${node.$p.npid}__*/${node.name}`;
  }
  return node.name;
}
...
function Program(node) {
  ...
  return node.body.map((n,i) => {
    return (printPids ? '/* stmt(' + n.$p?.npid + '): */ ' : '') + $(n,i);
  }).join('\n');
}
  ...
  if (printVarTyping && node.id.name) {
    ...
    if (meta?.typing) {
      ...
      if (init.type === 'FunctionExpression') {
        ...
        let allVals = '';
        if (meta?.allAssignedValues) {
          const tmp = Array.from(meta.allAssignedValues).sort((a,b) => a < b ? -1 : a > b ? 1 : 0).map(v => typeof v === 'string' ? JSON.stringify(v) : String(v)).join(' | ');
          allVals = tmp.slice(0, 100) + (tmp.length > 100 ? '...' : '');
        }
        return (
          node.kind + ' ' +
          $(node.id) +
          (
            ...
            meta.typing.mustBeType
              ? '/*:' + meta.typing.mustBeType + (allVals ? ': ' + allVals : '') + '*/ '
              : meta.typing.mustBePrimitive
                ? '/*:primitive' + (allVals ? ': ' + allVals : '') + '*/ '
                : '/*:unknown' + (allVals ? ': ' + allVals : '') + '*/ '
          ) +
          (
