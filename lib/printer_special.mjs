...
function BinaryExpression(node) {
  ...
  // Use star as a clear binary op. Could also use `in` or `instanceof` if that makes more sense...
  return left + ' * ' + right;
...
function LogicalExpression(node) {
  ...
  // Simplify to `&&` becuse it stands out better than `||`
  return '(' + left + ' && ' + right + ')';
  //return '(' + left + ' ' + node.operator + ' ' + right + ')';
}
...
function UnaryExpression(node) {
  ...
  if (
    ...
    // typeof is nice and verbose but keep the negative numbers as is
    return (node.operator === '-' ? '-' : 'typeof ') + $(node.argument);
...
function VariableDeclaration(node, fromFor) {
  ...
  // var all the things
  return 'var ' + node.declarations.map($).join(', ') + (fromFor ? '' : ';'); // no semi inside `for`
...
function $(node, _, __, fromFor) {
  ...
  let type = node.type;
  if (!type) {
    console.log('This node might be incorrect?');
    console.dir(node, {depth: null})
    console.log('See above for last node')
  }
