...
export default function walk(f, node, nameOfPropWhereNodeCameFrom='root') {
  ...
  function ww(f, node, prop, index = -1, blockRoot) {
    ...
    if (!stop && nodes.length > 1) {
      while (!stop) {
        const n2prop = nodes[nodes.length - 2][prop]; // top is current node so -2 is parent node
        // If previous visit removed the node and it was the last, return.
        if (index >= 0 && index >= n2prop.length) {
          stop = true;
          break;
        }
        // Make sure to get the current value of the node, in case it got replaced by the visitor
        const nowNode = index >= 0 ? n2prop[index] : n2prop;
        if (node === nowNode) break;

        if (typeof nowNode !== 'object') {
          console.dir('now dumping prop and index:');
          console.dir(prop);
          console.dir(index);
          console.dir(nowNode);
          console.dir(n2prop.length);
    ...
    if (!stop) {
      ...
      switch (t) {
        ...
        case 'SuperCall': {
          onlyObjects(node, ['arguments']);
          ww(f, node.arguments, 'arguments');
          break;
        }

        case 'SuperMethodCall': {
          onlyObjects(node, ['arguments', 'property', 'computed']);
          ww(f, node.arguments, 'arguments');
          ww(f, node.property, 'property');
          break;
        }
        ...
        case 'SuperProp': {
          onlyObjects(node, ['computed', 'property']);
          ww(f, node.property, 'property');
          break;
        }
        ...
        default:
          console.trace('[lib/walk] error, unexpected node type:', [t], node);
