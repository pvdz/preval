...
export default function walk(f, node, prop){
  ...
  function ww(f, node, prop, index = -1) {
    if (node === null) {
      return;
    }
    if (typeof node !== 'object') {
      console.dir(node);
      throw new Error('Should only walk nodes or `null` values, but `parent.' + prop + (index >= 0 ? '[' + index + ']' : '') + '` -> ' + node);
    }

    if (node instanceof Array) {
      return node.forEach((node, i) => ww(f, node, prop, i));
    }
