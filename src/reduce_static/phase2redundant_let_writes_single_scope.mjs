...
function _redundantWrites(fdata) {
  ...
  fdata.globallyUniqueNamingRegistry.forEach(function (meta, name) {
    ...
    vgroup('-- let: `' + name + '`, writes:', meta.writes.length, ', reads:', meta.reads.length);

    // Since this is a single scoped let the rwOrder should start with the var, unless this is TDZ
    const varWrite = meta.rwOrder[0];
    if (varWrite.type !== 'write' || varWrite.kind !== 'var') {
      vlog('First ref was not the var, this must be TDZ?');
      return;
    }
