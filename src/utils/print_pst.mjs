...
function printPstNode(node, indent, config) {
  ...
  switch (node.type) {
    ...
    case 'exportStar': return printStatement(indent, config, node);
...
function printExpression(indent, config, node) {
  switch (node?.type) {
    ...
    case 'FunctionExpression': {
      return `${node.isAsync ? 'async ' : ''}function${node.name ? ` ${node.name.name}` : ''}${node.isGenerator ? ' *' : ''}${node.params.length === 0 ? '()' : `(${node.params.map(p => printSimpleOrSpread(indent, config, p))} )`} ${printBlock(indent, config, node.body)}`;
    }
