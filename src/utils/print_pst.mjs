...
function _printStatement(indent, config, node) {
  switch (node.type) {
    ...
    case 'ExportNamed': {
      return `export { ${node.pairs.map(pair => `${printRef(indent, config, pair[0])} as ${pair[1]}`).join(',')} from "${node.source}"`;
    }
    ...
    case 'ImportNamed': {
      return `import { ${node.pairs.map(pair => `${pair[0]} as ${printRef(indent, config, pair[1])}`).join(', ')} from "${node.source}";`;
    }
    case 'LabelStatement': {
...
function printExpression(indent, config, node) {
  switch (node?.type) {
    ...
    case 'CallMethodExpression': {
      return `${printRef(indent, config, node.callee)}.${node.method}${callArgs(indent, config, node)}`
    }
