...
At the time of adding this tofix the file had something like this:

const tmpClusterSSA_a /*:unknown*/ = tmpNestedComplexRhs;
$(tmpNestedComplexRhs);
$(tmpClusterSSA_a);

but the const, assuming it's not used any further, is a useless alias since
the other binding cannot possibly change by passing it on in a func call.
it must be equal to do the second call with the same var when theres no observable
code in between.
