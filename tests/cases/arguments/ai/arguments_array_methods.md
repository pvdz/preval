...
# arguments_array_methods.md

> Arguments > Ai > Arguments array methods
>
> Test array methods on arguments

## Input

`````js filename=intro
function testArgsArrayMethods() {
...
testArgsArrayMethods(1, 2, 3, 4, 5);
`````
...
`````js filename=intro
const testArgsArrayMethods /*:()=>undefined*/ = function (/*uses arguments*/) {
...
`````js filename=intro
const testArgsArrayMethods = function () {
...
`````js filename=intro
let testArgsArrayMethods = function () {
  const tmpPrevalAliasArgumentsAny = arguments;
  debugger;
