...
# arguments_bind_variations.md

> Arguments > Ai > Arguments bind variations
>
> Test various bind patterns with arguments

## Input

`````js filename=intro
function testBindVariations() {
...
`````js filename=intro
const testBindVariations /*:()=>undefined*/ = function (/*uses arguments*/) {
...
`````js filename=intro
const testBindVariations = function () {
...
`````js filename=intro
let testBindVariations = function () {
  const tmpPrevalAliasArgumentsAny = arguments;
  debugger;
