...
# arguments_parameter_shadowing_apply.md

> Arguments > Ai > Arguments parameter shadowing apply
>
> Test parameter shadowing with apply calls

## Input

`````js filename=intro
function testArgsParameterShadowingApply() {
...
testArgsParameterShadowingApply(10, 20, 30);
`````
...
`````js filename=intro
const testArgsParameterShadowingApply /*:()=>undefined*/ = function (/*uses arguments*/) {
...
`````js filename=intro
const testArgsParameterShadowingApply = function () {
...
`````js filename=intro
let testArgsParameterShadowingApply = function () {
  const tmpPrevalAliasArgumentsAny = arguments;
  debugger;
