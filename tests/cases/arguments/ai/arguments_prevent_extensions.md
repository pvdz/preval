...
# arguments_prevent_extensions.md

> Arguments > Ai > Arguments prevent extensions
>
> Test Object.preventExtensions on arguments

## Input

`````js filename=intro
function testArgsPreventExtensions() {
...
testArgsPreventExtensions(1, 2, 3);
`````
...
`````js filename=intro
const testArgsPreventExtensions /*:()=>undefined*/ = function (/*uses arguments*/) {
...
`````js filename=intro
const testArgsPreventExtensions = function () {
...
`````js filename=intro
let testArgsPreventExtensions = function () {
  const tmpPrevalAliasArgumentsAny = arguments;
