...
# arguments_prototype_chain.md

> Arguments > Ai > Arguments prototype chain
>
> Test prototype chain checks on arguments

## Input

`````js filename=intro
function testArgsPrototypeChain() {
...
`````js filename=intro
const testArgsPrototypeChain /*:()=>undefined*/ = function (/*uses arguments*/) {
...
`````js filename=intro
const testArgsPrototypeChain = function () {
...
`````js filename=intro
let testArgsPrototypeChain = function () {
  const tmpPrevalAliasArgumentsAny = arguments;
  debugger;
