...
# every_callback_throws.md

> Array methods > Every > Ai > Every callback throws
>
> Test: Array.every with callback that throws

## Input

`````js filename=intro
const x = [1,2,3].every(function(x) { $(x); if (x === 2) throw new Error('stop'); });
$(x);
`````
