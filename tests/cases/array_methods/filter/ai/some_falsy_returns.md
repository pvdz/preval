...
> Array methods > Filter > Ai > Some falsy returns
>
> Test: Array.filter with falsy return values - should work with any truthy value

## Input

`````js filename=intro
const arr = $([0, '', false, null, undefined, 1]);
const result = arr.filter(x => x);  // Should return true because of the 1
$(result);
`````
...
$(
  $dotCall(tmpMCF, arr, `filter`, function (x) {
    return x;
  }),
);
`````
