...
> Array methods > Filter > Ai > Some mutate length up
>
> Test: Array.filter with length increase during iteration
...
let arr = [1,2];
let result = [];
const x = arr.filter(function(x, i, a) { result.push(x); if (i === 1) a.length = 4; });
$(result, x);
`````
