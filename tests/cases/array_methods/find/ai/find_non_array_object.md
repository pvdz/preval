...
> Array methods > Find > Ai > Find non array object
>
> Test: Array.find on array-like object

## Input

`````js filename=intro
const x = Array.prototype.find.call({0:'a',1:'b',length:2}, function(x) { $(x); });
$(x);
`````
...
const x /*:unknown*/ /*truthy*/ = $dotCall($array_find, tmpMCP, undefined, tmpMCP$1);
$(x);
`````
...
$($dotCall($array_find, { [0]: `a`, [1]: `b`, length: 2 }, undefined, tmpMCP$1));
`````
...
const d = $dotCall( $array_find, c, undefined, a );
$( d );
`````
