...
> Array methods > FindIndex > Ai > Find callback throws
>
> Test: Array.findIndex with callback that throws

## Input

`````js filename=intro
const x = [1,2,3].findIndex(function(x) { $(x); if (x === 2) throw new Error('stop'); });
$(x);
`````
