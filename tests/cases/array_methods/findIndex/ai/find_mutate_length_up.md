...
> Array methods > FindIndex > Ai > Find mutate length up
>
> Test: Array.findIndex with length increase during iteration
...
let arr = [1,2];
let result = [];
const x = arr.findIndex(function(x, i, a) { result.push(x); if (i === 1) a.length = 4; });
$(result, x);
`````
