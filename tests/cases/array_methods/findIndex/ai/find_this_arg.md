...
> Array methods > FindIndex > Ai > Find this arg
>
> Test: Array.findIndex with thisArg parameter
...
const result = arr.findIndex(function(x) {
...
const result /*:unknown*/ = $dotCall(tmpMCF, arr, `findIndex`, tmpMCP, obj);
$(result);
`````
...
$($dotCall(tmpMCF, arr, `findIndex`, tmpMCP, { threshold: 2 }));
`````
...
const i = { threshold: 2 };
const j = $dotCall( c, b, "findIndex", d, i );
$( j );
`````
...
const result = $dotCall(tmpMCF, arr, `findIndex`, tmpMCP, obj);
$(result);
`````
