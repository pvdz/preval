...
> Array methods > Findlast > Ai > Find callback args
>
> Test: Array.findLast callback arguments (value, index, array)

## Input

`````js filename=intro
const arr = $([1, 2, 3]);
const result = arr.findLast((value, index, array) => {
...
$(
  $dotCall(tmpMCF, arr, `findLast`, function $pcompiled(value, index, array) {
    if (value === 2) {
      if (index === 1) {
...
const l = $dotCall( c, b, "findLast", d );
$( l );
`````
