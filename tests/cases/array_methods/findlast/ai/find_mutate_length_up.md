...
> Array methods > Findlast > Ai > Find mutate length up
>
> Test: Array.findLast with length increase during iteration
...
let arr = [1,2];
let result = [];
const x = arr.findLast(function(x, i, a) { result.push(x); if (i === 1) a.length = 4; });
$(result, x);
`````
