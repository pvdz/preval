...
> Array methods > Findlastindex > Ai > Find callback args
>
> Test: Array.findLastIndex callback arguments (value, index, array)

## Input

`````js filename=intro
const arr = $([1, 2, 3]);
const result = arr.findLastIndex((value, index, array) => {
...
const k = $dotCall( c, b, "findLastIndex", d );
$( k );
`````
