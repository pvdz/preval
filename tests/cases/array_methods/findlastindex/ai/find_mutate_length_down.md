...
> Array methods > Findlastindex > Ai > Find mutate length down
>
> Test: Array.findLastIndex with length decrease during iteration
...
let arr = [1,2,3,4];
let result = [];
const x = arr.findLastIndex(function(x, i, a) { result.push(x); if (i === 1) a.length = 2; });
$(result, x);
`````
