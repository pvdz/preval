...
> Array methods > Findlastindex > Ai > Find this arg
>
> Test: Array.findLastIndex with thisArg parameter
...
const result = arr.findLastIndex(function(x) {
...
const result /*:unknown*/ = $dotCall(tmpMCF, arr, `findLastIndex`, tmpMCP, obj);
$(result);
`````
...
$($dotCall(tmpMCF, arr, `findLastIndex`, tmpMCP, { threshold: 2 }));
`````
...
const i = { threshold: 2 };
const j = $dotCall( c, b, "findLastIndex", d, i );
$( j );
`````
...
const result = $dotCall(tmpMCF, arr, `findLastIndex`, tmpMCP, obj);
$(result);
`````
