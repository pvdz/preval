...
> Array methods > Findlastindex > Ai > Find this value
>
> Test: Array.findLastIndex with arrow function (no thisArg binding)
...
let ctx = {mult: 2};
let result = [];
const x = [1,2,3].findLastIndex((x) => { result.push(typeof this); }, ctx);
$(result, x);
`````
