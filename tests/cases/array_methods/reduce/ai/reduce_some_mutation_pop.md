...
# reduce_some_mutation_pop.md

> Array methods > Reduce > Ai > Reduce some mutation pop
>
> Test: Array.reduce with pop during iteration
...
let arr = [1,2,3];
let result = [];
const x = arr.reduce(function(x) { result.push(x); arr.pop(); });
$(result, x);
`````
