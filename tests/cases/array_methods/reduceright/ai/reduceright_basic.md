...
# reduceright_basic.md

> Array methods > Reduceright > Ai > Reduceright basic
>
> Test: Array.reduceRight basic usage

## Input

`````js filename=intro
let result = [];
const x = [1,2,3].reduceRight(function(x) { result.push(x); });
$(result, x);
`````
