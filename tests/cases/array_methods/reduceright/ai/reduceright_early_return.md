...
# reduceright_early_return.md

> Array methods > Reduceright > Ai > Reduceright early return
>
> Test: Array.reduceRight early return - should stop after finding first match
...
const arr = $([1, 2, 3, 4, 5]);
let count = 0;
const result = arr.reduceRight(x => {
    count++;
...
const arr = $([1, 2, 3, 4, 5]);
let count = 0;
const result = arr.reduceRight(function (x) {
  count = count + 1;
...
const i = $dotCall( d, b, "reduceRight", e );
$( c );
$( i );
`````
