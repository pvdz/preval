...
# reduceright_mutation_push.md

> Array methods > Reduceright > Ai > Reduceright mutation push
>
> Test: Array.reduceRight with push during iteration
...
let arr = [1,2,3];
let result = [];
const x = arr.reduceRight(function(x) { $('pushing another x'); result.push(x); arr.push(x+10); });
$(result, x);
`````
