...
# reduceright_elided_empty_with_init.md

> Array methods > Reduceright > Reduceright elided empty with init
>
...
`````js filename=intro
let result = [];
const x = [,,,,,,].reduceRight(function(x) { result.push(this === undefined); }, 100);
$(result, x);
`````
...
const x = $dotCall(tmpMCF, tmpMCOO, `reduceRight`, tmpMCP, 100);
$(result, x);
`````
