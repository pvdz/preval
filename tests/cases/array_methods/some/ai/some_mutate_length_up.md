...
> Array methods > Some > Ai > Some mutate length up
>
> Test: Array.some with length increase during iteration
...
let arr = [1,2];
let result = [];
const x = arr.some(function(x, i, a) { result.push(x); if (i === 1) a.length = 4; });
$(result, x);
`````
