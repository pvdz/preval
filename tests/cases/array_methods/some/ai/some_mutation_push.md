...
> Array methods > Some > Ai > Some mutation push
>
> Test: Array.some with push during iteration
...
let arr = [1,2,3];
let result = [];
const x = arr.some(function(x) { $('pushing another x'); result.push(x); arr.push(x+10); });
$(result, x);
`````
