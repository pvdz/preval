...
# number_toExponential_call_ctx_1arg.md

> Builtins cases > Ai number > Number toExponential call ctx 1arg
>
> Test Number.prototype.toExponential called with .call and 1 argument (fraction digits), using a context value

## Input

`````js filename=intro
const ctx = $(123.456);
const result = Number.prototype.toExponential.call(ctx, 2);
$(result); // Expected: "1.23e+2"
`````
...
`````js filename=intro
$($dotCall($number_toExponential, $(123.456), undefined, 2));
`````
