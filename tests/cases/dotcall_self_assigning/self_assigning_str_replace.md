...
let tmpClusterSSA_count /*:number*/ = 66;
const arr /*:array*/ /*truthy*/ = [``, ``, ``, ``, ``, ``, ``, `x5C`, `x35`, `x36`, ``, `x31`];
while ($LOOP_NO_UNROLLS_LEFT) {
  const tmpPostUpdArgIdent$2 /*:number*/ = tmpClusterSSA_count;
  tmpClusterSSA_count = tmpClusterSSA_count - 1;
  if (tmpPostUpdArgIdent$2) {
    const chr$1 /*:primitive*/ = arr[tmpClusterSSA_count];
    if (chr$1) {
      const regex$1 /*:regex*/ /*truthy*/ = new $regex_constructor(`xyz`, `g`);
      const chr2$1 /*:primitive*/ = arr[tmpClusterSSA_count];
...
let tmpClusterSSA_count = 66;
const arr = [``, ``, ``, ``, ``, ``, ``, `x5C`, `x35`, `x36`, ``, `x31`];
while (true) {
  const tmpPostUpdArgIdent$2 = tmpClusterSSA_count;
  tmpClusterSSA_count = tmpClusterSSA_count - 1;
  if (tmpPostUpdArgIdent$2) {
    if (arr[tmpClusterSSA_count]) {
      str = $dotCall($string_replace, str, `replace`, new $regex_constructor(`xyz`, `g`), arr[tmpClusterSSA_count]);
    }
  } else {
    break;
  }
