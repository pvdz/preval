...
const f = function (inputstr$1) {
  ...
  if (0 < len) {
    const tmpClusterSSA_outputstr = tmpFree$1(repl$2);
    return tmpClusterSSA_outputstr;
  } else {
    return ``;
  }
...
const f = function ($$0) {
  ...
  if (test) {
    ...
    const c2 = $dotCall($string_indexOf, `ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=`, `indexOf`, c1);
    const tmpMCF$13 = repl.charAt;
    const d1 = $dotCall(tmpMCF$13, repl, `charAt`, 3);
    const tmpMCF$15 = $string_indexOf;
    const d2 = $dotCall($string_indexOf, `ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/=`, `indexOf`, d1);
    const frout = a2 + b2;
    outputstr = $coerce(frout, `plustr`);
    return outputstr;
  } else {
