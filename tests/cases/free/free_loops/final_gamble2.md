...
`````js filename=intro
let tmpClusterSSA_counter$2 = 22;
const arr = [
  `/`,
  `/`,
  `\u00c0`,
...
while ($LOOP_NO_UNROLLS_LEFT) {
  ...
  if (tmpIfTest$1) {
    const ARR_RESULT = arr[tmpClusterSSA_counter$2];
    const tmp$1 = $String_fromCharCode(ARR_RESULT);
    arr[tmpClusterSSA_counter$2] = tmp$1;
    tmpClusterSSA_counter$2 = tmpClusterSSA_counter$2 + 1;
  } else {
    break;
...
const tmpMCF = arr.join;
const tmpCalleeParam$1 = $dotCall(tmpMCF, arr, `join`, ``);
$(tmpCalleeParam$1);
`````
