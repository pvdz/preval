...
> Normalize > Expressions > Assignments > Arr spread > Auto ident nested member complex bin
>
...
throw `Preval: Array spread on non-string primitive must crash (caused by \`const tmpCalleeParam = [...7];\`)`;
`````
