...
> Normalize > Pattern > Assignment > Arr > Arr > Ident > Default yes yes no  arr 0
>
...
`````js filename=intro
throw `Preval: Array spread on non-string primitive must crash (caused by \`const tmpArrPatternSplat\$1 = [...0];\`)`;
`````
...
`````js filename=intro
const tmpArrAssignPatternRhs = [0, 4, 5];
