...
> Normalize > Pattern > Param > Obj > Obj > Default yes yes  empty str
>
...
let f = function (tmpParamDefault) {
  let { x: {} = $({ x: 'pass' }) } = tmpParamDefault === undefined ? $({ x: { y: 'fail2' } }) : tmpParamDefault;
  return 'ok';
};
$(f('', 10));
`````
