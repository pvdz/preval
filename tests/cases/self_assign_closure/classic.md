...
`````js filename=intro
// SHOULD inline self_closing_decoder
...
const data_arr = [`encodedgarble1`, `encodedgarble2`, `encodedgarble3`];
...
const k = [ "encodedgarble1", "encodedgarble2", "encodedgarble3" ];
const v = b;
while ($LOOP_DONE_UNROLLING_ALWAYS_TRUE) {
  try {
    const w = v( 477 );
...
$(tmpCalleeParam$53);
const tmpCalleeParam = data_arr[0];
const tmpCalleeParam$1 = data_arr[1];
$(tmpCalleeParam, tmpCalleeParam$1);
`````
